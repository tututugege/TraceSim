...
```bash
./a.out --mode trace ./test/dhrystone.bin --max-insts 10000000
```
...
```bash
./a.out --mode normal ./test/dhrystone.bin
```

## 参数说明

- `--mode <normal|trace|ckpt>`: 选择运行模式。
- `--image <path>`: 指定加载的裸机二进制镜像（.bin）。
- 位置参数兜底: 不写 `--image/--restore-file` 时，最后一个非选项参数会按模式解释：`ckpt` 模式视为 checkpoint 文件，其他模式视为 image。
